...
if page == "users":
    ...
    # ユーザー一覧
    url_users = "http://0.0.0.0:8080/users"
    ...
    # 会議室一覧取得
    url_rooms = "http://0.0.0.0:8080/rooms"
    ...
    url_bookings = "http://0.0.0.0:8080/bookings"
    ...
    with st.form(key="booking"):
        ...
        end_time = st.time_input(
            "終了時刻: ",
            value=datetime.time(hour=20, minute=0)
        )
    ...
    if submit_button:
        ...
        if booked_num > capacity:
            st.error(f"{room_name}の定員は{capacity}名以下の予約人数のみ受け入れています。")
        # 開始時刻 >= 終了時刻
        elif start_time >= end_time:
            st.error(f"開始実行が終了時刻を超えています。")
            ...
            # 会議室の予約を行う
            url = "http://0.0.0.0:8080/bookings"
            res = requests.post(url, data=json.dumps(data))
            if res.status_code == 200:
                st.success("予約完了")
